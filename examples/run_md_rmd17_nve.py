"""Run an NVE MD simulation starting from a random rMD17 frame.

Usage:
    python examples/run_md_rmd17_nve.py --checkpoint aspirin.mdl --molecule aspirin \\
        --n_steps 10000 --timestep 0.5 --output traj.xyz --log md.log

The starting frame is drawn from the rMD17 dataset (random by default,
or specify --frame_idx to pick a specific one). --temperature only seeds the
initial velocities; NVE then conserves total energy (no thermostat).
"""
...
def run_md_rmd17_nve(checkpoint, molecule='aspirin', data_dir=None,
    ...
    # ── Load starting frame ────────────────────────────────────────────────
    atoms = load_starting_frame(molecule, data_dir,
                                frame_idx, seed)
    ...
    # Logger — E_tot (= E_pot + E_kin) is the conserved quantity in NVE, so it's
    # logged explicitly as a drift/stability check.
    log_file = open(log, 'w')
    log_file.write(f"{'step':>8}  {'time_ps':>10}  {'E_pot (eV)':>14}  "
                   f"{'E_kin (eV)':>14}  {'E_tot (eV)':>14}  {'T (K)':>8}\n")
    log_file.flush()
    ...
    def log_step():
        ...
        line   = (f"{step:>8d}  {t_ps:>10.4f}  {e_pot:>14.6f}  {e_kin:>14.6f}  "
                  f"{e_tot:>14.6f}  {temp:>8.2f}")
    ...
    print(f"\nRunning {n_steps} NVE steps (initial T={temperature} K, "
          f"dt={timestep} fs)...\n")
...
def main():
    parser = argparse.ArgumentParser(description='NVE MD with ECENet on rMD17')
    ...
    parser.add_argument('--temperature',  type=float, default=300,
                        help='Initial temperature in K (seeds velocities; NVE conserves energy)')
